...
app.get('/myCommunity', async (req,res)=>{
    res.send(await User.myCommunity())
})

app.post('/addMyCommunity/:id/:username/:addId', async (req,res)=>{
    await User.addMyCommunity(req.params.id, req.params.username, req.params.addId)
    return res.send('Added')
})
